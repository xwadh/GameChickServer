...
describe('角色管理', function() {
    ...
    it('获取指定任务信息', function(done) {
        remote.auth({openid:"baaaaabbb"}).fetch({func:"task.getInfo",id:2002}, msg => {
            remote.log(msg);
            done();
        });
    });
    it('获取用户信息', function(done) {
        remote.auth({openid:"baaaaabbb"}).fetch({func:"query.getUserInfo",sql:'select * from users where status = 32',db:'chick_ios_1'}, msg => {
            console.log(msg);
            done();
        });
    })
