...
describe('内政', function() {
    ...
    it('获取总榜信息 - 9000', done => {
        remote.auth().fetch({func: "9000"}, msg => {
            remote.isSuccess(msg, true); //使用断言，对返回值进行合理性判定，如判定失败则抛出异常，下面的 done 就不会被执行
            msg.data.list.map(item=>{
                item.name = decodeURIComponent(item.name);
                remote.log(item);
    ...
    it('礼包 - getFesGift', function(done) {
        remote.auth({openid:"baaaasssabbb"}).fetch({func: "getFesGift",type:4}, msg => {
                remote.log(msg); //使用断言，对返回值进行合理性判定，如判定失败则抛出异常，下面的 done 就不会被执行
                done();
            })
    });
    it('礼包 - getFesInfo', function(done) {
        remote.auth({openid:"baaaasssabbb"}).fetch({func: "getFesInfo",type:"2"}, msg => {
            remote.log(msg); //使用断言，对返回值进行合理性判定，如判定失败则抛出异常，下面的 done 就不会被执行
            done();
        })
    });
