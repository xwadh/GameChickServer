...
let {UserStatus, ActionExecuteType, NotifyType, ReturnCode} = require('../const/comm');
...
describe('互动', function() {
    ...
    it('好友列表', done=>{
        remote.auth({openid:'777.492'}).fetch({func:'login.getFriendList'}, msg=>{
            msg.data.list.map(item=>{
     ...
     */
     it('消息系统 - 发消息', done =>{
        remote.watch(msg=>{
            ...
            remote.fetch({func:"mail.getList"}, msg => {
        ...
        Promise.all([
            ...
            rs[1].fetch({func:'sendHello', actionType: NotifyType.socialSendHello, openid: rs[0].userInfo.openid}, msg=>{
                ...
                setTimeout(()=>{
                    ...
                    }, NotifyType.friends).auth(null, msg=>{});
                }, 1000);
            });
        }).catch(e=>{});
    });
