...
class connectMonitor
{
     ...
     */
    execute(facade){
        if(!!facade.service.server){ // 清理僵尸连接
            //console.log('网络连接定时检测');

            let cur = (new Date()).valueOf();
            Object.keys(facade.service.server.connected).map(it=>{
                let conn = facade.service.server.connected[it];
            ...
            //更新当前在线数
            facade.numOfOnline = Object.keys(facade.service.server.connected).length;
        }
        else{ //通讯服务异常，重启中...
