...
let TaskConditionManager = require('../logic/TaskConditionManager');
let BonusList = {};
...
class TaskObject {
    ...
    GetBonus($user){
        $user.getBonus($user.convertBonus(BonusList[this.id]));
     ...
     */
    loadFromStatic(){
        let $staticStr = constList.StaticTaskList[this.id].split('|');
        $staticStr[0].split(';').map($value2=>{
        ...
        if(!BonusList[this.id]){
            BonusList[this.id] = $staticStr[1];//缓存奖励字符串 
        }
    }
