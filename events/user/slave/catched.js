...
function handle(data) {
    ...
    msg.info.time = this.DataConst.slave.catchTime;
    ...
    if(msg.info.src == data.user.openid){//抓捕者
        if(msg.info.code == ReturnCode.Success){
            ...
            if(msg.info.code == ReturnCode.Success){        
                ...
                let fri = data.user.baseMgr.txFriend.getFriend(msg.info.dst);
                if(!!fri && !this.sysCur.debug) {
        ...
        if(msg.info.code == ReturnCode.Success){
            ...
            if(msg.info.code == ReturnCode.Success){
                this.service.mails.add(data.user, msg, "system", data.user.openid);//写报告
