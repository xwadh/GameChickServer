 ...
 * 每日活动管理
 */
let { ReturnCode, NotifyType, BonusType, DailyActivityStatus } = require('../const/comm');
let FacadeOfLogic = require('../facade/FacadeOfLogic');
let FacadeOfIndex = require('../facade/FacadeOfIndex');
...
class dailyactivity {
    ...
    async loadDb() {
        ...
        let uset = await this.parent.cacheMgr.groupKeys('act');
        if (!!uset) {
            for (let $id of uset) {
                let us = await this.parent.cacheMgr.get($id);
                if (!!us) {
    ...
    async addProp(domain, openid, $choose, $num) {
        ...
        if (!uinfo) {
            ...
            //todo:改为随机奖励
            let bonus = [{ type: BonusType.Money, num: Math.floor(Math.random()*500) }];
            // bonus.push(this.parent.baseMgr.txFriend.getRandomBonus(false));
            //向用户发送一封邮件
            this.parent.remoteCall(si, "userNotify", {
                domain: si.domain,
                openid: si.openid,
                msg: {
    ...
    async getBonus() {
        ...
        for (let uo of sorted) {
            ...
            if(uo.bonus != 0){
                let bonus = [{ type: BonusType.Diamond, num: uo.bonus }];
    ...
    checkStatus() {
        ...
        // let tm1 = Date.parse(new Date("August 25,2017 0:0:0"));
        // let tm2 = Date.parse(new Date("August 28,2017 17:0:0"));
        // let tm3 = Date.parse(new Date("August 29,2017 0:0:0"));
        // let tm4 = Date.parse(new Date("August 29,2017 12:0:0"));
        let cur = new Date();
        if (!!this.id) {
        ...
        // let allow = (cur.getHours() < 14 && cur.getHours() >= 12) ? true : false;     //12点至14点为活动预热阶段，只有该时段可投奖
        ...
        // let update = (cur.getFullYear() == 2017 && cur.getMonth() == 8 && cur.getDate() == 4) ? true : false;                             //2017.9.4不开启活动
        // if(update){
        //     this.state = DailyActivityStatus.Idle;
        // }
    }
    ...
    async CheckButtonStatus(domain, openid) {
        ...
        if (!!ui) {
            let cur = new Date();
            // let tm = new Date("August 28,2017 17:0:0");
            if (this.state == DailyActivityStatus.Idle) {
                this.parent.remoteCall(ui, "userNotify", {
                    domain: domain,
                    openid: openid,
