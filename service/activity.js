 ...
 * 活动管理服务
 */
let {ReturnCode, ActivityScoreRate, NotifyType, BonusType, ActivityType, ActivityStatus, ActivityRankBonus, ActivityScoreBonus,ActivityRankMax} = require('../const/comm');
let StatusMachine = require('javascript-state-machine');

/**
 * 活动管理类
 */
class activity extends StatusMachine
{
    /**
     * 构造函数
     * @param {*} parent    门面对象
            ...
            methods: {
                ...
                onEnd:   () => {
                    ...
                    for(let uo of this.sorted){
                        ...
                        for(let rank in ActivityRankBonus[this.type]){
                            if(uo.rank <= rank){
                                //发放奖励，并下行奖励通知 - 改为事件模式
                                let user = this.parent.service.users.GetUser(uo.uid);
                                if(!!user){
        ...
        if($id == 0){ //领取排名奖
            ...
            for(let rank in ActivityRankBonus[this.type]){
                if($ui.rank <= rank){
                    if(!$user.baseMgr.vip.readActivityBonus(0)){
                        ...
                        $user.getBonus(ActivityRankBonus[this.type][rank].bonus);
                        return {code: ReturnCode.Success, data:{rank: $ui.rank, bonus:ActivityRankBonus[this.type][rank].bonus}};
                    }
                    else{
            ...
            if($id - 1 <= $ui.lv){
                if(!$user.baseMgr.vip.readActivityBonus($id)){
                    ...
                    $user.getBonus(ActivityScoreBonus[this.type][$id - 1].bonus);
                    return {code: ReturnCode.Success, data:{id: $id, bonus:ActivityScoreBonus[this.type][$id - 1].bonus}};
                }
                else{
                    return {code: ReturnCode.activityBonusGot};
                }
     ...
     */
    async loadDb(){
        let it = await this.parent.models.system().findCreateFind({
            where:{
                id:1
            },
            defaults: {activity:"{}"},
    ...
    getUserOrDefault($uid){
        if(!this.users.has($uid)){//如果不存在指定记录，就创建一条新记录
            this.users.set($uid, {
                uid:$uid,       //玩家ID
                aid:this.id,    //活动ID
                openid:this.parent.service.users.GetUser($uid).openid,
                name:this.parent.service.users.GetNameById($uid),
                icon:this.parent.service.users.GetUser($uid).baseMgr.info.GetHeadIcon(),
                score:0,        //活动积分
                lv:0,           //分段积分进度
                rank:0,         //排名，0表示未上榜
            });
    ...
    rankList($uid) {
        ...
        // let ret = {score:ui.score, rank: ui.rank, list:[], cur:$page, total: Math.ceil(this.sorted.length/10.0)};
        let ret = {list:[]};
        //todo:改为获取前三名，以及自身排名与其前后5名——共14位玩家排行
    ...
    addScore($uid, $type, $added){
        ...
        let user = this.parent.service.users.GetUser($uid);
        if(!!user){
            ui.score += $added * ActivityScoreRate[this.type]; //不同活动具有不同的分数转化率
            for(let i = 0;i < 5;i++){
                if(!!ActivityScoreBonus[this.type][i] && ui.score >= ActivityScoreBonus[this.type][i].score){
                    ui.lv = i;
                }
            }
            this.parent.notifyEvent("user.Activity.ScoreBonus", {user:user, score: ui.score, lv: ui.lv});
            // if(!!ActivityScoreBonus[this.type][ui.lv+1] && ui.score >= ActivityScoreBonus[this.type][ui.lv+1].score){
            //     //提升积分等级
            //     ui.lv++;
            //     //发放积分奖励 - 改为事件模式
            //     this.parent.notifyEvent("user.Activity.ScoreBonus", {user:user, score: ui.score, lv: ui.lv});
