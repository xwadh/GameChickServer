...
let {ActionExecuteType, UserStatus, NotifyType, ActivityType, BonusType, ReturnCode} = require('../const/comm');
...
class slave extends baseMgr {
    ...
    lash(openid){
        this.CheckStatus();
        if(!this.parent.baseMgr.txFriend.getFriend(openid)){ //没有发现指定好友
            ...
            if(!slv){//当前对象未被抓捕
                ...
                if(ret.code == ReturnCode.Success){
                    let fri = this.parent.baseMgr.txFriend.getFriend(openid);
                    // 奴隶收到鞭挞推送消息
                    if(!!fri) {
    ...
    ransom(openid){
        ...
        if(!this.parent.baseMgr.txFriend.getFriend(openid)){ //没有发现指定好友
            ...
            if(!master){ 
                ...
                //检测并扣除所需道具
                let $cost = Math.ceil((master.time - commonFunc.now()) / 3600.0);
                if(this.parent.baseMgr.info.GetRes(BonusType.Diamond) >= $cost){
                    this.parent.baseMgr.info.SubRes(BonusType.Diamond, $cost); 
                    let fri = this.parent.baseMgr.txFriend.getFriend(openid);
                    // 奴隶主收到赎身推送消息
                    if(!!fri) {
    ...
    flattery(openid){
        ...
        if(!this.parent.baseMgr.txFriend.getFriend(openid)){ //没有发现指定好友
            ...
            let ret = this.parent.baseMgr.item.useItem(403, 1);
            if(ret.code == ReturnCode.Success){
                if(this.parent.baseMgr.action.GetExecuteNum(ActionExecuteType.slaveFlattery)<=3){
                    mst.time -= this.parent.router.DataConst.slave.catchTime * 0.05;
                }
                this.CheckStatus();
                let fri = this.parent.baseMgr.txFriend.getFriend(openid);
                // 奴隶主收到谄媚推送消息
            ...
            return [ret.code, mst.time - commonFunc.now()];
        }
    }
    /**
     * 判断是否可逃脱
    ...
    canCatch(openid){
        ...
        else if(!this.parent.router.sysCur.debug && this.v.slave.length >= this.MaxSlave){ //奴隶数量达到上限
    ...
    calcSlaveTime(t){
        let $now = commonFunc.now();
        return Math.ceil(Math.max(0, $now - (t - this.parent.router.DataConst.slave.catchTime)) / 3600.0);
    }
    fullSlaveTime(t){
        return t >= this.parent.router.DataConst.slave.catchTime;
    }
    ...
    removeSlaveByIdx(idx){
        ...
        if(time > 0){
            ...
            msg.info.bonus = [{type:BonusType.Money, num:50*time}];
            if(this.fullSlaveTime(time)){
                msg.info.bonus.push({type:BonusType.Item, id:22, num:20});
            }
        }
        this.parent.router.service.mails.add(this.parent, msg, "system", this.parent.openid);
        //删除奴隶
    ...
    addMaster(openid){
        ...
        if(this.v.master.length == 0){
            ...
            //添加新的主人条目
            this.v.master.push({openid:openid, time: this.parent.router.DataConst.slave.catchTime + commonFunc.now()});
            this.dirty = true;
    ...
    addSlave(openid){
        ...
        if(!this.parent.router.sysCur.debug && this.v.slave.length >= this.MaxSlave){
            return ReturnCode.socialMaxSlave;
        }
            ...
            this.v.slave.push({openid:openid, time: this.parent.router.DataConst.slave.catchTime + commonFunc.now()});
            this.dirty = true;
     ...
     */
    get MaxSlave(){
        if(this.parent.baseMgr.vip.hisGateNo < 5){
            return 0;
        }
        else if(this.parent.baseMgr.vip.hisGateNo >= 5 && this.parent.baseMgr.vip.hisGateNo < 20){
            return 2;
        }
        else if(this.parent.baseMgr.vip.hisGateNo >= 20 && this.parent.baseMgr.vip.hisGateNo < 35){
            return 4;
        }
        else if(this.parent.baseMgr.vip.hisGateNo >= 35 && this.parent.baseMgr.vip.hisGateNo < 50){
            return 10;
        }
        else{
            return 20;
    ...
    getList(){
        ...
        this.v.slave.map(item=>{
            ret.slave.push({openid: item.openid, time: item.time - commonFunc.now()});
        });
        this.v.master.map(item=>{
            ret.master.push({openid: item.openid, time: item.time - commonFunc.now()});
        });
        return ret;
    }
}
