...
class social extends baseCtl {
        ...
        switch($msg.type){
            ...
            case NotifyType.slaveCommend: //奴隶：表扬
            {
                ...
                if(user.baseMgr.action.Execute(ActionExecuteType.slaveCommend, 1, true)) {
                    ...
                    // }
                    //成功后给奴隶主发放奖励
                    let bonus = [{type:BonusType.Money, num:500}];
                    user.getBonus(bonus);
            ...
            case NotifyType.slaveAvenge:
                {
                    ...
                    if($msg.info.code == ReturnCode.Success){
                        if(user.baseMgr.action.Execute(ActionExecuteType.slaveAvenge, 1, true)) {
                            ...
                            //产生并发放300gold
                            let bonus = [{type:BonusType.Money, num:300}];
                            user.getBonus(bonus);
                            $msg.info.bonus = bonus;
                        }
                        else {
            ...
            case NotifyType.slaveLash:{
                    ...
                    if($msg.info.code == ReturnCode.Success){
                        if(user.baseMgr.action.Execute(ActionExecuteType.slaveLash, 1, true)) {
                            ...
                            let bonus = [{type:BonusType.Diamond, num:num}];
                            user.getBonus(bonus);
                            $msg.info.bonus = bonus;
                            user.socialNotify($msg, objData.openid);
                        }
    ...
    async share(user, objData){
        ...
        //判断是否可进行首次分享操作
        if(!user.baseMgr.info.CheckStatus(this.parent.const.UserStatus.isFirstShare)){
            //标识用户已经进行过首次分享的操作了，该状态会被持久化到数据库，客户端可根据data.info.status进行判断
            user.baseMgr.info.SetStatus(this.parent.const.UserStatus.isFirstShare);

            //todo:向用户发放首次分享奖励，并notify给客户端
            user.getBonus({type:BonusType.Money, num:1000});
            user.getBonus({type:BonusType.Item, id: 20, num: 1});
            ret.code = ReturnCode.Success;
            ret.data.bonus = [{type:BonusType.Money, num:1000},{type:BonusType.Item, id:20, num:1}];
        ...
        switch(parseInt(objData.type)){
            case ActionExecuteType.AE_SocialOfFail:
                if(user.baseMgr.action.Execute(ActionExecuteType.AE_SocialOfFail, 1, true)){
                    user.baseMgr.task.Execute(this.parent.const.em_Condition_Type.shareOfFail, 1, this.parent.const.em_Condition_Checkmode.add);
                    user.baseMgr.task.Execute(this.parent.const.em_Condition_Type.totalShare, 1, this.parent.const.em_Condition_Checkmode.add);
            ...
            case ActionExecuteType.AE_SocialOfAction:
                if(user.baseMgr.action.Execute(ActionExecuteType.AE_SocialOfAction, 1, true)){
                    user.baseMgr.task.Execute(this.parent.const.em_Condition_Type.shareOfLackAction, 1, this.parent.const.em_Condition_Checkmode.add);
                    user.baseMgr.task.Execute(this.parent.const.em_Condition_Type.totalShare, 1, this.parent.const.em_Condition_Checkmode.add);
