...
let {DomainType, BonusType, ReturnCode} = require('../const/comm');
let baseCtl = require('../facade/baseCtl');
let {now, ms, sign} = require('../util/commonFunc');

class thirdPart extends baseCtl {
    /**
    ...
    async txpay(req){
        ...
        if(!this.parent.service.txApi.checkPayCallbackSign(data, "/txpay.html")){
            return {ret: this.parent.const.ReturnCode.illegalData, msg:"数据非法"};
        }
        else{
            ...
            if(result == this.parent.const.ReturnCode.Success){
    ...
    async pay360(req){
        let params = {
            game_key: this.parent.sysCur[DomainType.D360].game_key, //采信己方预设值
        ...
        if(!params.sign == sign(params, this.parent.sysCur[DomainType.D360].game_secret)){ //签名校验错误
            return "error.auth";
        }
    ...
    async auth360(objData){
        ...
        ret.sign = sign(ret, this.parent.sysCur[DomainType.D360].game_secret);
        return ret;
    }
    ...
    async authAdmin(objData){
        objData = this.parent.registerSession(objData.openid, objData.openkey, objData.session);
        if(!!objData){
            let ret = {
                t: now(),             //当前时间戳，必须验证时间戳，暂定有效 期为当前时间前后 5 分钟
                nonce: Math.random()*1000 | 0,              //随机数
                plat_user_id: objData.openid,               //平台用户 ID
                nickname: objData.openid,                   //用户昵称
                avatar: objData.openid,                     //头像
                is_tourist: 1,                              //是否为游客
                session:objData.session,                    //Session
            };
            ret.sign = this.parent.tools.sign(ret, this.parent.sysCur.game_secret);
            return ret;
        }
        else{
            return {session:""};
    ...
    async mf(req) {
        ...
        let pUser = this.parent.service.users.GetUser(this.parent.service.users.GetIdByDomainId(domain+"."+openid));
        if(pUser){
            //	写入账号信息
            pUser.WriteUserInfo(app_id, server_id, time, sign);
        }
        return this.parent.const.ReturnCode.Success;
    }

    async pay(req) {
        return this.parent.const.ReturnCode.Success;
        //MoreFun平台，暂时封闭
        // FormDataDeal(req, function(fields, files) {
        //     try {
        //         let q	 			= fields;
        //         //	获取购买信息
        //         let trade_no		= q.trade_no;
        //         let uuid            = q.uuid;
        //         let product_id      = q.product_id;
        //         let total_fee       = q.total_fee;
        //         let notify_time     = q.notify_time;
        //         let product_name    = q.product_name;
        //         let request_count   = q.request_count;
        //
        //         console.log('[debug] req=' + JSON.stringify(q, null, 2));
        //
        //         //	购买道具
        //         this.parent.control.login.CommitTrade(trade_no, uuid, product_id, total_fee).then(ret=>{
        //             return this.parent.const.ReturnCode.Success;
        //         }).catch(e=>{
        //             return this.parent.const.ReturnCode.Error;
        //         });
        //     } catch (e) {
        //         console.log('[error] deal buy error=' + String(e));
        //         return this.parent.const.ReturnCode.Error;
        //     }
        // });
    }
...
exports = module.exports = thirdPart;
